...
/*A1:值捕获是把变量拷贝一份进 lambda，生命周期独立，更安全,引用捕获是直接引用外部变量，性能略好但必须保证变量在 lambda 执行时仍然存在，否则会出现悬空引用,确定生命周期安全且需要共享修改时使用。
A2:std::function 可以统一封装任意可调用对象,灵活性远高于函数指针，特别是支持带捕获的 lambda代价是存在类型擦除带来的性能损耗以及可能的堆内存分配
A3:std::move(cb) 是为了把 std::function 内部持有的资源直接转移到任务对象中，避免一次昂贵的拷贝
A4:231